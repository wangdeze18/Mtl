...
def parse(path):
    ...
    with open(path, "r",errors='ignore') as f:
        ...
        for i in range(int(num_objects)):
            ...
            input_ids = torch.tensor(tokenizer.convert_tokens_to_ids(small_tokens)).unsqueeze(0)  # Batch size 1
...
def parse_dir(data_dir,path_to_dir):
    ...
    for file in tqdm(files, "parsing {}".format(path_to_dir)):
        ...
        with open(data_dir + "/" + "extractfeatures/" + str(number), "wb", 1) as f:
