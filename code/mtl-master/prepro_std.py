...
import json
import sys
from data_utils import load_data,load_split_data
...
def main(args):
    ...
    for task in task_defs.get_task_names():
        ...
        for split_name in task_def.split_names:
            ...
            rows = load_split_data(file_path, task_def, split_name)
            dump_path = os.path.join(mtl_root, "%s_%s.json" % (task, split_name))
