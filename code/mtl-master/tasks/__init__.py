...
class MTLTask:
    ...
    @staticmethod
    def train_forward(sequence_output, pooled_output, premise_mask, hyp_mask, decoder_opt, dropout_layer, task_layer,is_train):
        if decoder_opt == 1:
...
def register_task(name):
    ...
    .. note::
        ...
        All Tasks must implement the :class:`~MTLTask`
        interface.
    ...
    def register_task_cls(cls):
        ...
        if not issubclass(cls, MTLTask):
            raise ValueError('Task ({}: {}) must extend MTLTask'.format(name, cls.__name__))
